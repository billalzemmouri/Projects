            ...
            public void onClick(View v) {
                ...
                if (pseudo.length() > 0 && email.length() > 0 && password.length() > 0 && password2.length() > 0) {
                    if (!
                        ...
                        Toast.makeText(getApplicationContext(), "Les mots passe ne sont pas identiques", Toast.LENGTH_SHORT).show();
                    }
                    else {
                        request.register(pseudo, email, password, new MyRequest.RegisterCallBack() {
                            @Override
